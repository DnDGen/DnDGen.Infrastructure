...
namespace DnDGen.Infrastructure.Selectors.Collections
{
    internal class CollectionSelectorEventDecorator : ICollectionSelector
    {
        ...
        {
            eventQueue.Enqueue("Infrastructure", $"Selecting all collections from {tableName}");
            var collections = innerSelector.SelectAllFrom(tableName);
            eventQueue.Enqueue("Infrastructure", $"Selected {collections.Count()} collections from {tableName}");
        ...
        {
            eventQueue.Enqueue("Infrastructure", $"Finding collection to which {entry} belongs from {tableName}");
            var collectionName = innerSelector.FindCollectionOf(tableName, entry, filteredCollectionNames);
            eventQueue.Enqueue("Infrastructure", $"{entry} belongs to {collectionName}");
        ...
        {
            eventQueue.Enqueue("Infrastructure", $"Exploding {collectionName} from {tableName}");
            var explodedCollection = innerSelector.Explode(tableName, collectionName);
            eventQueue.Enqueue("Infrastructure", $"Exploded {collectionName} into {explodedCollection.Count()} entries");
        ...
        {
            eventQueue.Enqueue("Infrastructure", $"Flattening {collections.Count} collections with {keys.Count()} keys");
            var flattenedCollection = innerSelector.Flatten(collections, keys);
            eventQueue.Enqueue("Infrastructure", $"Flattened {collections.Count} collections into {flattenedCollection.Count()} entries");
