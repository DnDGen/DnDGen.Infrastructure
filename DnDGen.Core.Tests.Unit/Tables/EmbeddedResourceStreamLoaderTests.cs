...
namespace DnDGen.Core.Tests.Unit.Tables
{
    ...
    public class EmbeddedResourceStreamLoaderTests
    {
        ...
        {
            Assert.That(() => streamLoader.LoadFor("invalid filename.xml"), Throws.InstanceOf<FileNotFoundException>().With.Message.StartsWith("invalid filename.xml does not exist in DnDGen.Core.Tests.Unit"));
        }

        [Test]
        public void MatchWholeFileName()
        {
            Assert.That(() => streamLoader.LoadFor("Table.xml"), Throws.InstanceOf<FileNotFoundException>().With.Message.StartsWith("Table.xml does not exist in DnDGen.Core.Tests.Unit"));
        }
    }
}
