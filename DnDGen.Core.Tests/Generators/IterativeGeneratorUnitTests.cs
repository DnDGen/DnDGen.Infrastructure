...
namespace DnDGen.Core.Tests.Generators
{
    ...
    public class IterativeGeneratorUnitTests
    {
        ...
        public void GenerateWithLambda()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue("Core", "Beginning iterative generation"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Core", "Completed iterative generation after 1 iterations"), Times.Once);
        }
        ...
        public void RegenerateIfInvalid()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue("Core", "Beginning iterative generation"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Core", "(Attempt 1) 0 is not valid"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Core", "(Attempt 2) 1 is not valid"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Core", "Completed iterative generation after 3 iterations"), Times.Once);
        }
        ...
        public void ReturnDefault()
        {
            ...
            for (var i = 0; i < generator.MaxAttempts; i++)
                mockEventQueue.Verify(q => q.Enqueue("Core", $"(Attempt {i + 1}) {i} is not valid"), Times.Once);
        }
        ...
        public void ReturnValidObjectAfterTooManyRetries()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue("Core", "Beginning iterative generation"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Core", "Completed iterative generation after 42 iterations"), Times.Once);
