...
namespace DnDGen.Infrastructure.Tests.Unit.Selectors.Collections
{
    ...
    public class CollectionSelectorEventDecoratorTests
    {
        ...
        public void LogEventsForAllSelection()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Selecting all collections from table name"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Selected 2 collections from table name"), Times.Once);
        }
        ...
        public void LogEventsForCollectionNameSelection()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(2));
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Finding collection to which item belongs from table name"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"item belongs to inner group"), Times.Once);
        }
        ...
        public void LogEventsForExplodeCollection()
        {
            ...
            Assert.That(collection, Is.EqualTo(explodedCollection));
            mockEventQueue.Verify(q => q.Enqueue(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(2));
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Exploding collection name from table name"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Exploded collection name into 2 entries"), Times.Once);
        }
        ...
        public void LogEventsForExplodeCollectionWithDuplicates()
        {
            ...
            mockEventQueue.Verify(q => q.Enqueue(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(2));
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Exploding collection name from table name"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Exploded collection name into 3 entries"), Times.Once);
        }
        ...
        public void LogEventsForFlattenCollection()
        {
            ...
            Assert.That(collection, Is.EqualTo(flattenedCollection));
            mockEventQueue.Verify(q => q.Enqueue(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(2));
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Flattening 3 collections with 7 keys"), Times.Once);
            mockEventQueue.Verify(q => q.Enqueue("Infrastructure", $"Flattened 3 collections into 2 entries"), Times.Once);
        }
